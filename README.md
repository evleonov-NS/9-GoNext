...
## Ритуал этапа (чат → проверка → кип)

Порядок для **каждого** этапа из `PLAN.md`:

1. **Новый этап — новый чат.** Не продолжать этап N+1 в чате, где закрыли N.
2. Агент реализует этап, готовит доки/бамп версии **без** коммита.
3. Агент пишет **подробный чек-лист**, что проверить в приложении.
4. Владелец проверяет и пишет **«кип»** / **«ок на кип»**.
5. Только после этого: коммит + пуш вехи на `origin`.
6. Следующий этап — снова **новый чат**.

Правило также в `PLAN.md`, `AGENTS.md`, `STATUS.md`, `.cursor/rules/etapa-ritual.mdc`.

## Metro / Expo (агент)

- Запуск только из корня репозитория: `F:\Projects\Cursor\Work\9-GoNext`.
- Порт **8081**. Если занят — агент сам освобождает порт и перезапускает Metro (`npx expo start -c --port 8081`), без перехода на 8082 и без вопросов владельцу.
- После установки native-зависимостей агент сам перезапускает Metro.

## Завершение этапа: завершаем этап

По закрытии **каждого** этапа из `PLAN.md` (после «кип» владельца):
...
Без «кип» коммит/пуш вехи не делать. Без коммита/пуша этап не закрыт для учёта.
...
| Файл | Роль |
|------|------|
| `PLAN.md` | этапы, чек-листы, сметы; **ритуал этапа** (чат → проверка → кип) |
| `STATUS.md` | решения, прогресс, открытые вопросы; **полный статус** (таблица 0–10) |
...
| `AGENTS.md` | Expo v54 + ритуал этапа для агента |
| `.cursor/rules/etapa-ritual.mdc` | always-on правило ритуала в Cursor |

### Команда «полный статус»

Владелец пишет в чат: **полный статус**. Агент отвечает таблицей из раздела «Полный статус» в `STATUS.md` (этапы 0→10: статус, версия вехи, план, факт + строка «Итого» и следующий этап). При закрытии этапа таблицу обновлять там же.
